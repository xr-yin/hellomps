...
class LPTN(MPO):
    ...
    def gen_random_state(cls,N:int,m_max:int, k_max:int, phy_dims:list):
        ...
        As = []
        for i in range(N):
            size = (bond_dims[i],bond_dims[i+1],phy_dims[i],krauss_dims[i])
    ...
    def correlations(self, A, B, i):
        ...
        for j in range(i+1, N):
            ...
            Bob = np.tensordot(amp.conj(), Bob.swapaxes(2,3), axes=4)
            corrs[j-i] = Bob
            # update Alice
            Alice = np.tensordot(Alice, amp, axes=([1], [0]))
            Alice = np.tensordot(amp.conj(), Alice, axes=([0,2,3], [0,2,3]))
